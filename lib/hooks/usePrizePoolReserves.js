...
import { useGovernanceTimelockAaveDaiBalance } from 'lib/hooks/useGovernanceTimelockAaveDaiBalance'
...
export const usePrizePoolReserves = () => {
  ...
  const { data: aaveDaiRewardsBalance, isFetched: isAaveDaiRewardsBalanceFetched } =
    useGovernanceTimelockAaveDaiBalance()
  ...
  const adai = {
    ...
    token: {
      ...aaveDaiRewardsBalance,
      address: aDAI_ADDRESS
    }
  }
  data[NETWORK.mainnet]?.push(adai)

  const USDC_RESERVES_IN_V4 = 959883
  const USDC_TREASURY_IN_V4 = 2838909
